...
class BandpowerExtraction():
    ...
    def transform(self,X,y=None):
        ...
        final_data = np.zeros((n_instances,n_channels,int((n_timepoints-self.window_width)/self.window_space)+1))
        for instance in range(n_instances):
            for channel in range(n_channels):
                for timepoint in range(1,n_timepoints-self.window_width,self.window_space):
                    ...
                    final_data[instance][channel][int(timepoint/self.window_space)]=value
        return final_data
