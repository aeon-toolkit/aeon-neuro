...
def time_all_dataloading(return_type="nested_univ"):
	...
	for d in datasets:
		...
		X, y=load_UCR_UEA_dataset(name=d,
								  extract_path="C:\Code\sktime-neuro\sktime-neuro"
											   "\data", return_type=return_type)
		load_time = int(round(time.time() * 1000)) - start
		print(d," shape  = ",X.shape," loaded into", return_type, " ie. ", type(X),
			  "took ", load_time, " to load")
		times[d]=load_time
